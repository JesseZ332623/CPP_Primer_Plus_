...
bool isPalindromeString(std::string & __str)
{
    ...
    */
    if (__str.size() == 0) 
    {
        loger(std::cout, WARNING, "Invald Input!\n");
        return false;
    }
...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    loger(std::cout, NOTIFY, "This Program Check The String You Input Is PalindromeString or Not.\n");
    
    std::string userInput;

    loger(std::cout, NOTIFY, "Please enter something: ");

    while (std::getline(std::cin, userInput) && (userInput != ".quit"))
    {
        if (isPalindromeString(userInput))
        {
            loger(std::cout, CORRECT, "String: [", userInput, "] is PalindromeString!\n");
        }
        else
        {
            loger(std::cout, WARNING, "String: [", userInput, "] is not a PalindromeString!\n");
        }

        loger(std::cout, NOTIFY, "Please enter something again: \n");
    }
